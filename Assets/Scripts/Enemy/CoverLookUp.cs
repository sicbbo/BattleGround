...
public class CoverLookUp : MonoBehaviour
{
    ...
    private Vector3[] GetSpots(GameObject go, LayerMask obstacleMask)
    {
        ...
        foreach(Collider col in go.GetComponents<Collider>())
        {
            ...
            {
                Vector3 extents = go.GetComponent<MeshCollider>().bounds.extents;
                float maxBounds = extents.z + extents.x;
            ...
            if (Vector3.Equals(go.transform.localScale, Vector3.one))
            {
    ...
    private ArrayList FilterSpots(StateController controller)
    {
        ...
        for (int i = 0; i < allCoverSpots.Count; i++)
        {
            ...
            foreach(Vector3 spot in allCoverSpots[i])
            {
                ...
                if (vectorDist.sqrMagnitude <= controller.viewRadius * controller.viewRadius && Physics.Raycast(spot, vectorDist, out RaycastHit hit,
                    vectorDist.sqrMagnitude, controller.generalStats.coverMask))
                {
                    if (hit.collider == covers[i].GetComponent<Collider>() && !TargetInPath(controller.transform.position, spot,
                        controller.personalTarget, controller.viewAngle / 4))
                    {
    ...
    {
        ArrayList nextCoverSpot = FilterSpots(controller);
        int nextCoverHash = (int)nextCoverSpot[0];
        float minDist = (float)nextCoverSpot[1];
