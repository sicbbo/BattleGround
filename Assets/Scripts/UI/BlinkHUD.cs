...
public class BlinkHUD : MonoBehaviour
{
    ...
    private void Update()
    {
        if (blink)
        {
            colorGradingLayer.saturation.value = Mathf.Lerp(0f, dyingSaturation, colorGradeTimer / blinkFadeStep);
            colorGradingLayer.brightness.value = Mathf.Lerp(0f, dyingBrightness, colorGradeTimer / blinkFadeStep);
            bloomLayer.intensity.value = Mathf.Lerp(0f, 7f, colorGradeTimer / blinkFadeStep);
            ...
            if (fadeTimer >= blinkFadeStep || fadeTimer <= 0f)
            {
                ...
                if (fadeTimer >= blinkFadeStep && lastTime)
                {
